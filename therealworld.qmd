...
main.path <- getwd()
users <- stream_in(file("C:/Data/The-real-world/Data/users.json", encoding = "UTF-8"))
#somehow strem_in doesn't work with partial file paths or paste0()...

original <- users #make a copy, just in case
...
Matching on all the municipality names leads to too many false positive matches (probably due to some places with short names such as Au, even though only full matches should be considered). Thus, let's create a manual list.
...
names <- c("switzerland", "swiss", "schweiz", "suisse", "svizzera", 
           ...
           "zurich", "geneva", "genève", "genf", "basel", "lausanne",
...
write.csv(CH.users, file=paste0(main.path, "/Code/CH_users.csv"))

```

With this (about 180 users), we can start adding some of the content. Let's load one of the "the real world" json files from the private channels.

```{r realworld, warning=FALSE, echo=FALSE, include=FALSE}
real.world.1 <- stream_in(file("C:/Data/The-real-world/Data/Private/The Real World/01H261HJTJ4K6RM2WWGW5PS40E.json"), 
...
write.csv(real.world.df, file=paste0(main.path, "/Data/real_world1.csv"))
